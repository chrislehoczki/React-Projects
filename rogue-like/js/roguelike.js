...
var weapons = [{name: "stick", damage: 3,ammo: 1000},
			  {name: "pistol", damage: 5, ammo: 10 },
			  {name: "shotgun", damage: 7, ammo: 5},
			  {name: "machine gun", damage: 8, ammo: 3},
			  {name: "bazooka", damage: 10, ammo: 2},
			  {name: "rail gun", damage: 15, ammo: 1}
];
...
var Board = React.createClass({
	...
	addPlayerSurrounds: function() {
		...
		board.map(function(tile) {
			if (tile.x > player.x -5 && tile.x < player.x + 5 && tile.y > player.y -5 && tile.y < player.y + 5) {
					...
					if (tile.class === "enemy" || tile.class === "boss") {
						...
						stateWeapons.forEach(function(weapon) {
							if (weapon.name === currentWeapon) {
								if (weapon.ammo === 0) {
									component.setState({message: "You have no ammo left for that weapon. You must choose another."})
									ammoLeft = false;
								}
								else {
									weapon.ammo -= 1;
						...
						if (tile.life > 0) {
							...
							var damage = (randomiseAttack(component.state.attack) * (component.state.level) + 1)
							tile.life -= damage;
		
							component.setState({health: component.state.health - randomiseAttack(tile.strength)})
							component.setState({weapons: stateWeapons, currentWeapon: currentWeapon}, console.log(component.state))
							component.setState({message: "You did " + damage + " damage. Enemy Life: " + tile.life})

							if (component.state.health <= 0) {
								component.setState({alertStyle: "alert-danger", message: "Oh no! You died......"})
								setTimeout(function(){ component.startGame(); }, 3000);
								return;
							}
							...
							//DEAD - ADD XP
								...
								if (tile.class === "boss" && tile.life <= 0) {
									component.setState({message: "You completed the level, well done!", alertStyle: "alert-success"});
									setTimeout(function(){ component.startGame(); }, 3000);
								...
								if (component.state.nextLevel === 10) {
									component.setState({level: component.state.level + 1, nextLevel: 20})
								}
								else {
									component.setState({nextLevel: component.state.nextLevel -10})
								}
	...
	render: function() {
		...
		return (
			<div>
			<Title />
			<WeaponsSelect weapons={this.state.weapons} weapon={this.state.currentWeapon} onChange={this.changeWeapon} />
			<State board={this.state}/>
			<MessageBox message={this.state.message} alertStyle={this.state.alertStyle} />
			<Dungeon tiles={this.state.board} />
			<Button bsStyle="btn btn-primary" onClick={this.toggleBoard}> Get a Hint </Button>
			<Key />
			</div>
			)
...
var Key = React.createClass({
	...
	render: function() {
		return (
			<div className="key">
			<ul>
			<li> <div className="boss"></div> <p> Boss </p> </li>
			<li> <div className="enemy"></div> <p> Enemy </p> </li>
			<li> <div className="weapon"></div> <p> Weapon </p> </li>
			<li> <div className="life"></div> <p> Life </p> </li>
			</ul>
...
var Tile = React.createClass({
	...
	if (this.props.state.screenClass === "hide") {
		...
		return (
			<span style={style} className={this.props.state.class} ></span>
			)
	}
})
...
var WeaponsSelect = React.createClass({
	...
	render: function() {
		...
		this.props.weapons.map(function(weapon) {
			weaponOptions.push(<option key={weapon.name} value={weapon.name}> {weapon.name} </option>) 
		})
		 
		return (
			<div className="weapons-select">
				<p className="weapons-select-text"> Change Weapon </p>
				<select id="select-box" onChange={component.handleSelect} value={this.state.weapon} >
				{weaponOptions}
				</select>
			</div>
			)
